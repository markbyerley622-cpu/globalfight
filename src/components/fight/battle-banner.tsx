...
export function BattleBanner({ battle, fightDate }: { battle: BattleRoomDTO; fightDate: string }) {
  ...
  return (
    <div
      className={cn(
        "sticky top-0 z-10 rounded-card border bg-ink-950/95 p-3 backdrop-blur",
        iWon ? "border-up/40" : iLost ? "border-blood-500/40" : "border-ink-700",
      )}
    >
      <div className="mb-2.5 flex items-center justify-between gap-2">
        <span className="inline-flex items-center gap-1.5 text-3xs font-bold uppercase tracking-wider text-blood-300">
          <Swords className="size-3.5" />
          {settled ? "Battle settled" : battle.state === "WAITING" ? "Battle open" : "Your battle"}
        </span>
        <span className="inline-flex items-center gap-2">
          {battle.meetings > 0 && (
            <span className="text-3xs uppercase tracking-wider text-fog">
              {battle.meetings === 1 ? "First meeting" : `Met ${battle.meetings}×`}
            </span>
          )}
          {battle.streak.count > 0 && (
            <span className={cn("rounded px-1.5 py-0.5 text-3xs font-bold", battle.streak.mine ? "bg-up/15 text-up" : "bg-blood-500/15 text-blood-300")}>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-start gap-2">
        ...
        {battle.opponent ? (
          ...
          <p className="animate-pulse pt-1 text-right text-2xs text-fog">
      ...
      <div className="mt-3 flex items-center justify-between gap-2 border-t border-ink-800 pt-2.5">
        <span className="inline-flex items-center gap-1 text-3xs uppercase tracking-wider text-fog">
          {settled ? (
  ...
  return (
    <div className={cn("flex min-w-0 items-start gap-2", align === "right" && "flex-row-reverse text-right")}>
      ...
      <div className="min-w-0">
        ...
        <PickLine identity={identity} className="text-2xs" />
