  ...
  if (done) {
    return (
      <div className="mt-6 flex flex-col gap-3">
        <div className="rounded-card border border-up/40 bg-up/10 p-5 text-center">
        ...
        <div className="card-surface p-4">
          ...
          <label className="tap mt-3 flex cursor-pointer items-center justify-center gap-2 rounded-lg border-2 border-dashed border-ink-700 bg-ink-850 px-4 py-5 text-xs font-semibold text-mist hover:border-ink-600 hover:text-chalk">
          ...
          {proof && (
            <p className={`mt-2 flex items-center gap-1.5 text-2xs ${proof.state === "error" ? "text-down" : proof.state === "done" ? "text-up" : "text-mist"}`}>
    ...
    <form onSubmit={submit} className="mt-6 flex flex-col gap-3.5">
      <label className="block">
        ...
        <textarea
          ...
          className="w-full rounded-lg border border-ink-700 bg-ink-900 px-3.5 py-2.5 text-sm text-chalk placeholder:text-fog focus:border-ink-600 focus:outline-none"
        />
      </label>
      ...
      <label className="block">
        ...
        <textarea
          ...
          maxLength={300}
          rows={2}
          className="w-full rounded-lg border border-ink-700 bg-ink-900 px-3.5 py-2.5 text-sm text-chalk focus:border-ink-600 focus:outline-none"
        />
      </label>

      {error && (
        <p className="rounded-lg border border-down/40 bg-down/10 px-3.5 py-3 text-sm text-down">{error}</p>
      )}
