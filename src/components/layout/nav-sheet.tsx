    ...
    <Sheet open={open} onClose={onClose} title={t("Menu")}>
        ...
        {PRIMARY_NAV.map((item) =>
          item.children ? (
            <div key={item.href} className="pt-1">
              <div className={cn("px-3 pb-1 pt-2 font-display text-3xs font-bold uppercase tracking-widest", item.accent ? "text-blood-400" : "text-fog")}>
                {t(item.label)}
              </div>
      ...
      {legal && (
        <div className="mt-4 border-t border-ink-800 px-5 pt-4">
          <div className="pb-2 font-display text-3xs font-bold uppercase tracking-widest text-fog">
            {t(legal.title)}
          </div>
      ...
      <p className="mt-4 px-5 text-center text-3xs leading-relaxed text-fog">
