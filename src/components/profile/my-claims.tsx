      ...
      <div className="mt-3 flex flex-col gap-2">
        ...
        {data.owned.map((g) => (
          <Link
            ...
            className="flex items-center gap-3 rounded-card border border-up/30 bg-up/8 px-3.5 py-3 transition-colors hover:border-up/50"
          >
            ...
            <span className="shrink-0 font-display text-2xs font-bold uppercase tracking-wide text-up">Manage</span>
          </Link>
        ))}
        ...
        {claims.map((c) => {
          ...
          const Icon = s.icon;
          return (
            <div key={c.id} className="rounded-card border border-ink-700 bg-ink-850 px-3.5 py-3">
              <div className="flex items-center gap-2">
                ...
                <span className={cn("inline-flex shrink-0 items-center gap-1 rounded-md px-1.5 py-0.5 font-display text-3xs font-bold uppercase tracking-wider", s.tone)}>
                  <Icon className="size-3" /> {s.label}
                </span>
              </div>
              <p className="mt-1 text-2xs leading-relaxed text-fog">{c.note || s.help}</p>
            </div>
          );
        })}

        {data.owned.length === 0 && claims.length === 0 && (
          <p className="rounded-lg border border-dashed border-ink-700 px-3.5 py-4 text-center text-xs text-fog">
            You don&apos;t own or manage a gym yet.
          </p>
        )}

        <div className="flex flex-wrap gap-2">
          <Link href="/gyms" className="tap inline-flex items-center gap-1.5 rounded-lg border border-ink-700 px-3.5 py-2 font-display text-2xs font-bold uppercase tracking-wide text-mist hover:text-chalk">
            <Search className="size-3.5" /> Find &amp; claim a gym
          </Link>
          <Link href="/gyms/new" className="tap inline-flex items-center gap-1.5 rounded-lg border border-ink-700 px-3.5 py-2 font-display text-2xs font-bold uppercase tracking-wide text-mist hover:text-chalk">
