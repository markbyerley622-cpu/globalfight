...
import { Loader2, AlertCircle, Pencil, Trash2, Check, X, Wifi, Reply, Quote, CornerDownRight } from "lucide-react";
    ...
    <div>
      ...
      {loading ? (
        <div className="space-y-3">
          {[0, 1, 2].map((i) => <div key={i} className="h-28 animate-pulse rounded-card bg-ink-850/60" />)}
        </div>
      ) : posts.length === 0 ? (
        <div className="card-surface p-8 text-center text-sm text-fog">{t("No posts yet.")}</div>
      ) : (
        <div className="space-y-3">
      ...
      <div ref={composerRef} className="mt-6 scroll-mt-24">
        {locked ? (
...
function PostItem({
  post, isOp, canManage, requireAuth, locked, onReply, onQuote, onChanged,
}: {
  ...
  return (
    <div className={cn("card-surface p-4 sm:p-5", isOp && "border-blood-500/30")}>
...
function ReplyComposer({
  threadSlug, target, onClearTarget, onPosted,
}: {
  ...
  return (
    <form onSubmit={submit} className="card-surface p-4 sm:p-5">
      {target && (
      ...
      <textarea
        ...
        placeholder={t("Add your take… use @ to mention someone")}
        rows={3}
