...
export default function GymClaimsPage() {
  ...
  return (
    <div className="container-cr py-8">
      ...
      <div className="mt-4 flex gap-2">
        {TABS.map((t) => (
          <button
            ...
            onClick={() => setTab(t)}
            className={cn(
              "rounded-full border px-3.5 py-1.5 font-display text-2xs font-bold uppercase tracking-wide transition-colors",
              tab === t
      ...
      {error && <p className="mt-4 rounded-lg border border-down/40 bg-down/10 px-3.5 py-3 text-sm text-down">{error}</p>}

      {claims === null ? (
        <p className="mt-8 flex items-center gap-2 text-sm text-fog"><Loader2 className="size-4 animate-spin" /> Loading…</p>
      ) : claims.length === 0 ? (
        <p className="mt-8 rounded-lg border border-dashed border-ink-700 px-6 py-10 text-center text-sm text-fog">
          ...
          {claims.map((c) => (
            <li key={c.id} className="card-surface p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className={cn(
                  "shrink-0 rounded-md px-2 py-1 font-display text-3xs font-bold uppercase tracking-wider",
              ...
              <div className="mt-2.5 flex items-center gap-2">
                <code className="rounded border border-ink-800 bg-ink-950/70 px-2 py-1 font-mono text-2xs text-fog">
                  {c.id}
                </code>
                <button
                  type="button"
              ...
              {c.evidence && (
                <p className="mt-3 whitespace-pre-wrap rounded-lg border border-ink-800 bg-ink-950/60 p-3 text-xs leading-relaxed text-mist">
                  {c.evidence}
                </p>
              )}
              ...
              {c.evidenceUploadedAt ? (
                <div className="mt-2.5 flex flex-wrap items-center gap-2 rounded-lg border border-ink-800 bg-ink-950/60 p-3">
                  ...
                  {c.evidenceScanStatus && c.evidenceScanStatus !== "CLEAN" && c.evidenceScanStatus !== "SKIPPED" ? (
                    <span className="inline-flex items-center gap-1 rounded-md bg-gold-500/15 px-1.5 py-0.5 text-3xs font-bold uppercase tracking-wider text-gold-300">
                    ...
                    <a
                      ...
                      className="tap ml-auto rounded-lg border border-volt-500/40 bg-volt-500/10 px-2.5 py-1.5 font-display text-2xs font-bold uppercase tracking-wide text-volt-400"
                    >
                      View document
                    </a>
                  )}
                ...
                <p className="mt-2.5 rounded-lg border border-dashed border-ink-800 px-3 py-2 text-2xs text-fog">
                  No document attached — text evidence only.
                </p>
              )}
              {c.note && <p className="mt-2 text-2xs text-fog">Note: {c.note}</p>}
...
function Action({
  ...
  return (
    <button
      ...
      disabled={busy}
      className={cn(
        "tap inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 font-display text-2xs font-bold uppercase tracking-wide transition-colors disabled:opacity-60",
