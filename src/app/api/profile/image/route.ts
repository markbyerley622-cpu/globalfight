...
import { refuseIfProfileImagesDisabled } from "@/lib/ugc-guard";
...
export async function POST(req: Request) {
  ...
  // Gated on the PROFILE-image flag, not the blanket UGC-media one.
  //
  // This route was behind refuseIfUgcMediaDisabled(), which is off at launch to stop
  // unscanned FILES being published (clips, forum attachments). That reasoning does
  // not reach this path: the upload is magic-byte checked, then decoded and
  // re-encoded to WebP by sharp, so what lands in the bucket is generated by us and
  // the uploaded bytes are never stored or served. The blanket flag was therefore
  // making avatar and banner return 503 — a user could not set a profile picture at
  // all — to mitigate a threat this route does not carry. See lib/feature-flags.
  const disabled = refuseIfProfileImagesDisabled();
