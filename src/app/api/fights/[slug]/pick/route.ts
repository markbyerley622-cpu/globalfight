...
/** Cast/change the viewer's pick on a bout. Body: { corner: "RED"|"BLUE", confidence?: 1..5 } */
export async function POST(req: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    return NextResponse.json(await castPick(user.id, slug, body?.corner, body?.confidence));
  } catch (e) {
