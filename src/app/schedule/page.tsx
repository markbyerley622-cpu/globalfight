          ...
          return (
            <div key={e.id} className="card-surface overflow-hidden">
              <div className="flex flex-col gap-4 border-b border-ink-700 p-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    {/* One statement of the promotion: the mark, or its name
                        when there is no logo. Never both. */}
                    <PromotionLogo promotion={e.promotion} size="sm" showName />
              ...
              <div className="divide-y divide-ink-800">
                {e.fights.map((f) => (
                  <Link key={f.id} href={`/events/${e.slug}#fight-${f.slug}`} className="flex items-center justify-between gap-3 px-5 py-3 hover:bg-ink-800/50">
              ...
              <Link href={`/schedule/${e.slug}`} className="block border-t border-ink-700 px-5 py-3 text-center font-display text-xs font-semibold uppercase tracking-wide text-blood-400 hover:bg-ink-800/50">
